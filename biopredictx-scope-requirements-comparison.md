...
Requirement inventories for both scopes, extracted under a strict fidelity rule, plus a side-by-side comparison. Old scope: [[biopredictx-scope-v2]]. New scope: [[biopredictx-fdc-alpha-2-0]]. Parent: [[biopredictx-alpha-2-0]].

## Method

For each scope I listed every functional (FR) and non-functional (NFR) requirement, wrote a one-line definition drawn only from the source text, then validated that nothing was invented. **Rule applied: no new requirements created.** Where I imposed structure the document didn't (the FDC has no requirement numbering), the IDs are labels for stated criteria and are flagged as such. Inferences are tagged `[ASSUMPTION]`; risks and open items are listed separately from requirements.
...
| FR-03.1 | Curated clinical data lake — S3 Tables (Parquet), partitioned by date and cohort_id; raw landing-zone Glacier lifecycle. |
...
## Validation — no new requirements

- **Old scope:** 13 FR entries and 8 NFR entries, each traced to a cited FDRD code in the scope doc. No code was invented; ranges left as grouped; the undefined member NFR-01.1 was not given a fabricated definition.
- **New scope:** 11 FR and 9 NFR entries, each traced to a named FDC section. IDs are labels only. Two forward-looking items (NFR-A8/A9) flagged as Beta strategy, not Alpha requirements. Wearable selection and therapy-control-logic items held out as assumptions/risks, not requirements.
- **Result:** every listed requirement maps to source text; no requirement was extracted that the documents do not state.
...
The new scope is not a smaller version of the old one — it is a different system with a different center of gravity. The old scope's mass sat in engineered compliance and scale (de-identification, audit, DR, thundering-herd ingestion). The new scope drops nearly all of that and adds a capability the old scope never had: a **real-time closed-loop therapy controller that must fire before deep-sleep onset (NFR-A2)**. That single timing constraint, plus the open assumption that COTS staging can meet it, is the highest-risk item in Alpha 2.0 — the FDC itself flags that custom software may be required if the assumption fails.
