...
**Method:** incremental displacement, per [[patterns-of-legacy-displacement]]. The Phase 2 gate is now resolved in favour of displacement; the in-place refactor is retained as a documented fallback rather than a co-equal option — reasoning in §3.
...
**Path A is retained as a fallback**, documented at the end of §4, for the case where Phase 0 shows the product lines cannot be separated cleanly — for example if all four solutions prove to share a single inseparable Next Actions engine.
...
| 0.9 | **Reporting dependency map** — which Power BI reports bind to which OLTP objects, who owns each, and which outputs are business-critical vs. bloat | Now a Phase 2A input, not just a constraint register. Divert the Flow explicitly warns to decompose *who uses which output* before rebuilding, or you rebuild the bloat. |
...
| 0.11 | **Azure Migrate assessment** — discovery and assessment across the SQL estate before committing to any database target. Produce: target recommendation (in-place SQL Server 2022 / Managed Instance / Azure SQL Database / SQL Server on Azure VM), right-sizing and monthly cost estimate, feature-compatibility gap list | **No database target gets chosen without this.** Must enumerate the instance-level dependencies Azure SQL Database drops: **SQL Agent jobs** (→ elastic jobs), **linked servers**, **cross-database queries**, **Windows logins** (→ Entra ID), system-database usage, and any third-party agent needing OS/file-system access (Datto RMM/EDR, Veeam, the file server in the p.28 architecture) — Microsoft's stated trigger for choosing SQL Server on Azure VM instead. Consumes 0.2 and 0.4. |
| 0.12 | **Data flow map to ultimate source** — trace each key data flow past ProWeb to where it actually originates (client systems, EDI, ISO/Verisk), and capture what ProWeb discards or delays | Revert to Source prerequisite. The article's warning is that architecture diagrams stop at legacy; the value is in what lies behind it. Also identifies upstream data currently thrown away because ProWeb had nowhere to put it. |
| 0.13 | **Product line separability assessment** — can Discovery, Aged, Historical, and Day 1 be separated, or do they share one inseparable Next Actions engine? | Determines whether Extract Product Lines is viable at all, and therefore whether we are on the displacement path or the §4 fallback. p.17 suggests Discovery activates only ARC detection and coverage discovery, which would make it a genuinely thin slice — confirm. |
...
| # | Task | Notes |
|---|---|---|
| 1.1 | Consolidate source control to Azure DevOps; branch policies; **revoke direct-to-production push for third-party developers** | Governance fix and a named cyber gap. Resolve the TFS-vs-Azure-DevOps contradiction (§7) first. |
| 1.2 | Reproducible build → CI → automated deploy to lower environments | Build reproducibility precedes pipeline automation. |
| 1.3 | **Characterization / golden-master test harness over the stored procedure surface** | The enabling investment. Gates all of Phase 2. Sized off 0.2 and 0.3, not guessed. |
...
| 1.9 | Observability baseline — app and DB telemetry into a single monitoring plane | No SIEM today. Also gives us the change-impact signal we need during displacement. |
| 1.10 | **Database target decision and execution — gated on 0.11.** Default: in-place upgrade to SQL Server 2022 to clear 12 Oct 2027, cloud decision deferred | If cloud is chosen: **Managed Instance** unless the assessment shows no instance-level dependencies; migrate with **Azure DMS in online mode** or transactional replication (supports SQL Server 2017 as a source, with limitations on publishable objects). **Not BACPAC** — export time rises sharply with object count and it requires downtime. Use vCore for **Azure Hybrid Benefit**. Scale target resources up for cutover: transaction log rate is governed in Azure SQL Database. |
...
| # | Task |
|---|---|
| 2A.1 | Stand up a decoupled analytical store fed by CDC from ProWeb, plus **Revert to Source** feeds for anything ProWeb only passes through (per 0.12) |
| 2A.2 | Rebuild reports **iteratively, one at a time, to production** — not as a big-bang reporting migration |
| 2A.3 | **Parallel running with reconciliation** against the existing Power BI outputs, with alerting on divergence |
| 2A.4 | Hunt down **"off system" workarounds** — manual spreadsheet manipulation between ProWeb output and what leadership actually sees. Assume these exist and are undocumented |
| 2A.5 | Stage cutover by user cohort; retire the direct-to-OLTP query path per report |
| 2A.6 | Fix data quality issues **upstream**, not by reimplementing legacy workarounds in the new store |
...
| # | Task |
|---|---|
| 2B.1 | **Discovery (pilot).** New service implementation; **Event Interception** on ISO/Verisk webhooks and SFTP arrivals; **Revert to Source** for placement files. Deliverable includes the reusable scaffolding, not just the feature |
| 2B.2 | **Legacy Mimic** layer so clients, clearinghouses, and ISO/Verisk cannot tell which system is serving them |
| 2B.3 | **Aged (first real line, 32% of volume, 20–30% of collections).** Staged traffic cutover — beta cohort, then 1% / 5% / 10%, as in the insurance worked example |
| 2B.4 | **Historical** (25%) |
| 2B.5 | **Day 1** (42%) last — highest volume, deepest ownership, full lifecycle |
| 2B.6 | Progressive retirement of displaced ProWeb modules, contracted as named deliverables |
| 2B.7 | **Removal of transitional architecture** — façade, CDC bridges, reconciliation jobs. Funded and scheduled, not left to goodwill |
...
| # | Task |
|---|---|
| 3.1 | Triage the **60% unplanned** automation opportunity by ProWeb dependency and by automation *class* (see R8) |
...
| 3.6 | Point the AI/ML pipeline at source data rather than ProWeb SQL where 0.12 shows ProWeb is only passing data through | 
...
| # | Task |
|---|---|
| 4.1 | Extend the 2A analytical store into longitudinal claim analytics |
| 4.2 | Feature store for the ML pipeline |

### Fallback — in-place refactor (Path A)

If 0.13 shows the product lines cannot be separated, we fall back to extracting logic domain-by-domain inside ProWeb behind the 1.3 harness: trigger elimination, WCF → REST, .NET Core, incremental React islands. Phase 2A still comes first and still applies. This path is slower to value, commits us closer to feature parity, and should be priced per domain only. Its risks are in §5.

---

## 5. Risk register by task
...
| 0.13 Separability | The four lines share one inseparable Next Actions engine, so Extract Product Lines does not apply and we are on the fallback path | **M×H** | Same tables and same procedures serving all four lines with only flag-based variation | This is the single most consequential Phase 0 finding. Run it early enough that the answer arrives before we commit to a Phase 2 shape. |

### Phase 1

| Task | What goes wrong | L×I | Leading indicator | How we de-risk |
|---|---|---|---|---|
| 1.1 Source control | Migration loses history, or two sources of truth run in parallel and diverge | M×M | The TFS/ADO contradiction in §7 goes unresolved | Resolve current state first. Freeze, migrate, verify, cut over — no dual-write period. |
...
| 1.10 | Cloud migration and displacement run concurrently, so two foundations move at once and neither rolls back cleanly | **M×H** | Cloud migration scheduled inside the Phase 2 window | Sequence, do not parallelize. Clear the 2027 deadline in place; move to cloud when the architecture suits it. |
| 1.10 | In-place SQL Server 2022 upgrade surfaces deprecated T-SQL or compatibility-level behavior changes across thousands of procedures | **M×H** | No harness coverage on the affected procedures | Run Data Migration Assistant first. Pin compatibility level initially, raise it deliberately behind 1.3. |
...
| 2B.3 Aged cutover | Staged traffic shift lacks a genuine rollback path, so 10% becomes all-or-nothing under pressure | **M×H** | No tested rollback at the 1% stage | Prove rollback at 1% before going to 5%. The insurance example's gradient only works if each step is reversible. |
...
| 3.4 Agentic execution | An autonomous action creates a billing or compliance error at scale before anyone notices | **M×H** | Action library expanding faster than the audit trail | Bounded action library, action-level audit, reversibility, staged rollout by volume. PHI-minimized scoping is right — keep it. |
...
## 7. Contradictions in the source report to resolve
...
## 8. What we need before we price anything
