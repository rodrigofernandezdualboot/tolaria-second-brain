---
type: Project
status: Active
has:
  - "[[wg-henschen-rag-scope-call-notes]]"
---

# WG Henschen

Aerospace parts supplier with a large, complex inventory of components and part numbers. Returning client — Dualboot previously built a Windows native app that processes technical drawings (PDFs) into structured CSV/Excel via AWS Bedrock (~400 hrs, ~$80K).

Current opportunity: a RAG-based chat tool over NetSuite data, embedded in NetSuite, covering inventory, accounting, and customer domains. In pre-estimate discovery.

## Related

- [[wg-henschen-rag-scope-call-notes]] — scope call notes, architecture direction, open questions, next steps.
